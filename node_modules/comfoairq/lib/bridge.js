...
class ComfoAirQBridge extends events {
    ...
    initSocket() {
        ...
        this.sock.on('data', (data) => {
            // TCP fragmentation buffer fix — accumulate data for complete messages
            this._rxBuffer = Buffer.concat([this._rxBuffer, data]);
            let offset = 0;
            while (offset + 4 <= this._rxBuffer.length) {
                const msglen = this._rxBuffer.readInt32BE(offset);
                if (msglen <= 0 || msglen > 65535) {
                    this._rxBuffer = Buffer.alloc(0); break;
                }
                const totalLen = msglen + 4;
                if (offset + totalLen > this._rxBuffer.length) break;
                const buffer = this._rxBuffer.slice(offset, offset + totalLen);
                const rxdata = { 'time': new Date(), 'data': buffer, 'kind': -1, 'msg': null };
                if (this._settings.debug) this.logger(' <- RX : ' + buffer.toString('hex'));
                this.emit('received', rxdata);
                offset += totalLen;
            }
            this._rxBuffer = offset > 0 ? this._rxBuffer.slice(offset) : this._rxBuffer;
        });
